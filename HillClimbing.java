...
class HillClimbing {
    ...
    public boolean buscar(Nodo inicial, Nodo objetivo, List<Nodo> ruta) {
        ...
        while (!actual.getNombre().equals(objetivo.getNombre())) {
            ...
            // Si no se encuentra un vecino mejor, termina la búsqueda (estancamiento).
            if (mejorVecino == null) {
    ...
    // Método heurístico que calcula la "distancia" entre el nodo actual y el nodo objetivo.
    private int heuristica(Nodo nodo, Nodo objetivo) {
        // Supongamos que la heurística calcula una distancia abstracta basada en hash codes.
        // Esto es una simplificación para este ejemplo.
        return Math.abs(nodo.hashCode() - objetivo.hashCode());
    }
